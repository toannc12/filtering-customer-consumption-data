...
-- Tại mỗi năm chỉ lưu trữ ra 3 tháng có có Millions of Dollars cao nhất trong năm đó và sub-category 
--(value theo Millions of Dollars tại mỗi sub-catgory giảm dần)
...
WITH ranked AS (
    SELECT
        ...
        millions_of_dollar,
        pipeline_exc_datetime,
        ROW_NUMBER() OVER (PARTITION BY sub_category, EXTRACT(YEAR FROM aggregation_date) ORDER BY millions_of_dollar DESC) AS rank_num
...
WHERE r.rank_num <= 3
ORDER BY r.sub_category, yt.total DESC, EXTRACT(YEAR FROM r.aggregation_date) DESC, r.millions_of_dollar DESC;
